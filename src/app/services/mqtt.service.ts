...
import {IClientOptions, IClientPublishOptions, IClientSubscribeOptions, ISubscriptionGrant, MqttClient, IConnackPacket} from 'mqtt';
import {throwError, from, concat, BehaviorSubject, Observable, Subject, of} from 'rxjs';
        ...
        return this._status.pipe(
            switchMap(status => {
                if (status === ConnectionStatus.DISCONNECTED) {
                    return throwError(new Error('No connection with MQTT.'));
                }
